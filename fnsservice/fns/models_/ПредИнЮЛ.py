...
class ПредИнЮЛ(models.Model): #4.30
    ГРНДатаПерв = models.ForeignKey(ГРНДатаТип, null=True, blank=True, related_name='СвНомТел_Дата',
    ...
    СвНомТел = models.ForeignKey(СвНомТелТип, null=True, blank=True,
                             verbose_name='Сведения о контактном телефоне ФЛ', on_delete=models.DO_NOTHING)
...
class ПредИнЮЛCreate:
    ...
    def __init__(self, item):
        if item is not None:
            ...
            self.a =ПредИнЮЛ.objects.create(ГРНДатаПерв=self.ГРНДатаПерв.save(),
                                            СвФЛ=self.СвФЛ.save(),
