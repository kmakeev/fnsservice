...
class СвЗалогДержФЛ(models.Model): # 4.99
    ...
    СвФЛ = models.ForeignKey(СвФЛЕГРЮЛТип, null=True, blank=True, verbose_name='Сведения о ФИО и (при наличии) ИНН ФЛ', on_delete=models.DO_NOTHING)
...
class СвЗалогДержФЛCreate:
    ...
    def __init__(self, item):
        if item is not None:
            ...
            self.a = СвЗалогДержФЛ.objects.create(ГРНДатаПерв=self.ГРНДатаПерв.save(),
                                                    СвФЛ=self.СвФЛ.save(), СвРождФЛ=self.СвРождФЛ.save(),
                                                    УдЛичнФЛ=self.УдЛичнФЛ.save(), АдресМЖРФ=self.АдресМЖРФ.save(),
                                                    АдрМЖИн=self.АдрМЖИн.save(), СвНотУдДогЗал=self.СвНотУдДогЗал.save())
