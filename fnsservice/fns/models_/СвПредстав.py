...
class СвПредстав(models.Model): #4.56
    ...
    СвУчетНОПредстав = models.ForeignKey(СвУчетНОПодраздТип, null=True, blank=True,
                                verbose_name='Сведения об учете в налоговом органе по месту нахождения филиала', on_delete=models.DO_NOTHING)

    def __str__(self):
        return 'СвПредстав: %s' % (self.СвНаим,)


class СвПредставCreate:
    ...
    def __init__(self, item):
        if item is not None:
            ...
            self.СвУчетНОПредстав = СвУчетНОПодраздТипCreate(item.find('СвУчетНОПредстав'))

            self.a = СвПредстав.objects.create(ГРНДатаПерв=self.ГРНДатаПерв.save(),
                                               СвНаим=self.СвНаим.save(),
                                               АдрМНРФ=self.АдрМНРФ.save(),
                                               АдрМНИн=self.АдрМНИн.save(),
                                               СвУчетНОПредстав=self.СвУчетНОПредстав.save())
        else:
            self.a = None
